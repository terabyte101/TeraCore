...
{
public:
    PlayerbotChatHandler(Player* pMasterPlayer) : ChatHandler(pMasterPlayer->GetSession()) {}
    bool revive(Player& botPlayer) { botPlayer.ResurrectPlayer(1.0f); botPlayer.SpawnCorpseBones(); botPlayer.SaveToDB(); return true; }
    bool teleport(const Player &botPlayer){ return HandleSummonCommand(this, botPlayer.GetName().c_str()); }
